...
void ComputeLcLdaTransform(
  ...
  const Matrix<double> *conf_spk_mean_mat);
...
class LcpLDAClass {
 public:
  LcpLDAClass(BaseFloat total_covariance_factor,
            ...
            Matrix<double> *conf_spk_mean_mat):
  ...
  void operator () () {
    ComputeLcLdaTransform(i_, tcf_, lda_out_vec_,mean_spk_,tot_mean_spk_,weight_spk_,Nc_, within_covar_vec_, conf_spk_mean_mat_);
  }
  ~LcpLDAClass() { }
 private:
  BaseFloat tcf_;
  ...
  Matrix<double> *conf_spk_mean_mat_;
};
...
void ComputeLcpLdaTransform(
  ...
  for(spk_indx=0; spk_indx<num_spk_; spk_indx++){
    ...
    for(int32 i=0; i<num_utt_all; i++){
      if(distance_spk_utt_row(i) >= min_threshold_between)
        conf_spk_mean.AddVec(1.0, utts_of_all_spk.Row(i));
    }
    // sub the within spk 
  ...
  Matrix<double> conf_spk_mean_matd(conf_spk_mean_mat); //float to double 
...
int main(int argc, char *argv[]) {
  ...
  try {
    const char *usage =
        ...
        "e.g.: \n"
        " ivector-compute-lcplda --beta=0.1 --k1=10 --k2=1.2 ark:ivectors.ark ark:utt2spk ark:lclda.mat.ark ark:spk_mean.ark \n";
    ...
    ComputeLcpLdaTransform(utt2ivector,
                        spk2utt,
                        total_covariance_factor,
                        covariance_floor,
                        lda_mat_vec,
