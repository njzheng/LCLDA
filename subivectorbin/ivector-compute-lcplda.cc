...
void ComputeLcpLdaTransform(
  ...
  // for(int32 spk_indx=0; spk_indx<num_spk_; spk_indx++){
  //   const SubVector<BaseFloat> sub_Nc(N_c, 0, spk_indx+1); //origin , length 
  //   int32 length = (int) N_c(spk_indx);
  //   int32 end_index = (int) (sub_Nc.Sum() - 1) ;
  //   int32 beg_index = (int) (sub_Nc.Sum() - length);
  ...
  //   Matrix<BaseFloat> utts_of_this_spk(spk_utt);
  //   total_covar.AddMat2(weight_spk_row(spk_indx), utts_of_this_spk, kTrans, 1.0);
  //   // total_covar.AddMat2(1.0, utts_of_this_spk, kTrans, 1.0);
    
  //   utts_of_this_spk.AddVecToRows(-1.0, mean_spk->Row(spk_indx));
  //   within_covar.AddMat2(weight_spk_row(spk_indx), utts_of_this_spk, kTrans, 1.0);
