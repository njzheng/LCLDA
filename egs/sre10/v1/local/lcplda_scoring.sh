...
if [ "$use_existing_models" == "true" ]; then
  for f in ${lda_ivec_dir}/mean.vec ${lda_ivec_dir}/transform_lcp.ark ; do
    [ ! -f $f ] && echo "No such file $f" && exit 1;
  done
else
  mkdir -p ${lda_ivec_dir}/log
  run.pl ${lda_ivec_dir}/log/lcplda.log \
      ...
      "ark:ivector-normalize-length scp:${lda_ivec_dir}/ivector.scp ark:- |" \
        ark:${lda_ivec_dir}/utt2spk \
        ark:${lda_ivec_dir}/transform_lcp.ark \
...
run.pl $scores_dir/log/lcplda_scoring.log \
