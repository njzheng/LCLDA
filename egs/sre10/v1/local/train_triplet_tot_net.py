...
# construct network
subnet_list_name = sys.argv[3]+".list"
...
out_dim = 400
model = net.TriTotnet(subivector_dim, subnet_list, 600, 400, out_dim, device)
...
for epoch in range(niter):
    ...
    for i, data in enumerate(train_loader, 0):
        ...
        output1, output2, output3 = model(input,device)
    ...
    print('-----epoch: %d, vad loss: %.6f, output mean dis %.5f, min dis %.5f'
        %( epoch + 1, vad_loss, dis.mean(), dis.min()))
