...
if [ "$use_existing_models" == "true" ]; then
  for f in ${lda_ivec_dir}/mean.vec ${lda_ivec_dir}/transform.ark ; do
    [ ! -f $f ] && echo "No such file $f" && exit 1;
  done
else
  mkdir -p ${lda_ivec_dir}/log
  run.pl ${lda_ivec_dir}/log/clda.log \
    ivector-compute-clda --dim=$lda_dim  --beta=$beta --total-covariance-factor=$covar_factor \
    "ark:ivector-normalize-length scp:${lda_ivec_dir}/ivector.scp ark:- |" \
      ark:${lda_data_dir}/utt2spk \
      ${lda_ivec_dir}/transform_clda.mat \
      ark,t:${lda_ivec_dir}/spk_mean_clda.ark || exit 1;

fi

mkdir -p $scores_dir/log
run.pl $scores_dir/log/clda_scoring.log \
  ...
  "ark:ivector-normalize-length scp:${enroll_ivec_dir}/spk_ivector.scp ark:- |
    ivector-transform ${lda_ivec_dir}/transform_clda.mat ark:- ark:- |
    ivector-normalize-length ark:- ark:- |" \
  "ark:ivector-normalize-length scp:${test_ivec_dir}/ivector.scp ark:- | ivector-transform ${lda_ivec_dir}/transform_clda.mat ark:- ark:- | ivector-normalize-length ark:- ark:- |" \
