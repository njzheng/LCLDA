...
if [ "$use_existing_models" == "true" ]; then
  for f in ${lda_ivec_dir}/mean.vec ${lda_ivec_dir}/transform_lp.mat \
       ${lda_ivec_dir}/transform_lda_lcp.ark ${lda_ivec_dir}/spk_mean_ln_lda.ark; do
  ...
  run.pl ${lda_ivec_dir}/log/lda_lclda_plda1.log \
    ivector-compute-lplda --dim=$lda_dim1 --total-covariance-factor=$covar_factor \
      "ark:ivector-normalize-length scp:${lda_ivec_dir}/ivector.scp ark:- |" \
        ...
        ${lda_ivec_dir}/transform_lp.mat  || exit 1;
  ...
  run.pl ${lda_ivec_dir}/log/lda_lclda_plda2.log \
      ...
      "ark:ivector-normalize-length scp:${lda_ivec_dir}/ivector.scp ark:- |  transform-vec  ${lda_ivec_dir}/transform_lp.mat ark:- ark:- | ivector-normalize-length ark:-  ark:- |" \
      ark:${lda_data_dir}/utt2spk \
      ark:${lda_ivec_dir}/transform_lda_lcp.ark \
...
run.pl $scores_dir/log/lda_lcplda_scoring.log \
  ivector-compute-dot-products-lclda2  "cat '$trials' | cut -d\  --fields=1,2 |"  \
  "ark:ivector-normalize-length scp:${enroll_ivec_dir}/spk_ivector.scp  ark:-| transform-vec  ${lda_ivec_dir}/transform_lp.mat ark:- ark:- | ivector-normalize-length ark:-  ark:- |" \
  "ark:ivector-normalize-length scp:${test_ivec_dir}/ivector.scp ark:-| transform-vec  ${lda_ivec_dir}/transform_lp.mat ark:- ark:- | ivector-normalize-length ark:-  ark:- |" \
  "ark:${lda_ivec_dir}/transform_lda_lcp.ark" \
