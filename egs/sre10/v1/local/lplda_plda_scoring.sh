...
if [ "$use_existing_models" == "true" ]; then
  for f in ${plda_ivec_dir}/transform_lplda.mat ${plda_ivec_dir}/lplda_plda ; do
    [ ! -f $f ] && echo "No such file $f" && exit 1;
  done
else
  
mkdir -p ${plda_ivec_dir}/log
run.pl ${plda_ivec_dir}/log/lplda.log \
    ...
    "ark:ivector-normalize-length scp:${plda_ivec_dir}/ivector.scp ark:- |" \
      ...
      ${plda_ivec_dir}/transform_lplda.mat || exit 1;


  # Train a PLDA model. may need a sub global mean here
  $train_cmd $plda_ivec_dir/log/lplda_plda.log \
    ivector-compute-plda ark:$plda_data_dir/spk2utt \
    "ark:transform-vec  $plda_ivec_dir/transform_lplda.mat scp:$plda_ivec_dir/ivector.scp ark:- | ivector-normalize-length ark:-  ark:- |" \
     $plda_ivec_dir/lplda_plda || exit 1;
fi
...
$train_cmd $scores_dir/log/eval_scoring.log \
  ...
  "ivector-copy-plda --smoothing=0.0  $plda_ivec_dir/lplda_plda - |" \
  "ark:ivector-mean ark:$enroll_data_dir/spk2utt scp:$enroll_ivec_dir/ivector.scp ark:- | ivector-subtract-global-mean $plda_ivec_dir/mean.vec ark:- ark:- | transform-vec $plda_ivec_dir/transform_lplda.mat ark:- ark:- | ivector-normalize-length ark:- ark:- |" \
  "ark:ivector-subtract-global-mean $plda_ivec_dir/mean.vec scp:$test_ivec_dir/ivector.scp ark:- | transform-vec $plda_ivec_dir/transform_lplda.mat ark:- ark:- | ivector-normalize-length ark:- ark:- |" \
