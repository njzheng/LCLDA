...
if [ "$use_existing_models" == "true" ]; then
  for f in ${lda_ivec_dir}/transform_nda.mat ; do
    [ ! -f $f ] && echo "No such file $f" && exit 1;
  done
else
  
mkdir -p ${lda_ivec_dir}/log
run.pl ${lda_ivec_dir}/log/nda.log \
    ...
    "ark:ivector-normalize-length scp:${lda_ivec_dir}/ivector.scp ark:- |" \
      ...
      ${lda_ivec_dir}/transform_nda.mat || exit 1;
fi

mkdir -p $scores_dir/log
run.pl $scores_dir/log/nda_scoring.log \
  ...
  "ark:ivector-normalize-length scp:${enroll_ivec_dir}/spk_ivector.scp ark:- |
    ivector-transform ${lda_ivec_dir}/transform_nda.mat ark:- ark:- |
    ivector-normalize-length ark:- ark:- |" \
  "ark:ivector-normalize-length scp:${test_ivec_dir}/ivector.scp ark:- | ivector-transform ${lda_ivec_dir}/transform_nda.mat ark:- ark:- | ivector-normalize-length ark:- ark:- |" \
