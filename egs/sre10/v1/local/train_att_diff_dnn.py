# This is for subivector with attention and triplet loss 
...
if sys.argv.__len__() != 4 :
    print("The usage: python3 local/train_att_dnn.py gpu_device_indx train_path dnn_name")
    print(sys.argv[:])
    exit()
...
loss_name = "batch_hard_triplet_loss" # cross_entropy  triplet_loss
...
if(loss_name == "cross_entropy"):
    ...
    # scp_path = os.path.join(train_path,"ivector.scp")
    scp_path = os.path.join(train_path,"subivector.scp")

    # utt for select the same class and different class triplet samples
    uttid_list = list(train_utt2int.uttid_dic.values())
    # k_times*utt_in_class = triplet_number for each class
    all_dataset = dataset.SubIvector_Batch_Hard_Triplet_Dataset(scp_path, uttid_list, spk_num)
    # ivector_dim = np.size(all_dataset.data_list[0])
    ivector_dim = np.shape(all_dataset.data_list[0])[1]
    phone_num = np.shape(all_dataset.data_list[0])[0]
    ...
    print('The ivector_dim is %d, the phone number is %d' %(ivector_dim, phone_num))
...
# construct network
# model = net.Att_Diff_DNN2(ivector_dim, phone_num, ivector_dim*10, ivector_dim)
model = net.Att_Diff_DNN(ivector_dim, phone_num, ivector_dim*10, ivector_dim)
...
if(loss_name == "cross_entropy"):
    ...
    ivector_dim_sqrt = torch.tensor(ivector_dim).float()
    criterion = loss.Batch_hard_TripletMarginLoss(margin=0.01*ivector_dim_sqrt, p=2, k=1)
    criterion.to(device)
