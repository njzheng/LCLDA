...
if [ "$use_existing_models" == "true" ]; then
  for f in ${plda_ivec_dir}/mean.vec ${plda_ivec_dir}/transform_lda.mat \
  		 ${plda_ivec_dir}/transform_lda_lc.ark ${plda_ivec_dir}/spk_mean.ark \
  		 ${plda_ivec_dir}/lda_lclda_plda ; do
	...
	run.pl ${plda_ivec_dir}/log/lda_lclda_plda1.log \
		ivector-compute-lda --dim=$lda_dim1 --total-covariance-factor=$covar_factor \
			"ark:ivector-normalize-length scp:${plda_ivec_dir}/ivector.scp ark:- |" \
	  		...
	  		${plda_ivec_dir}/transform_lda.mat  || exit 1;
	...
	run.pl ${plda_ivec_dir}/log/lda_lclda_plda2.log \
    	ivector-compute-lclda-pdf --nj=$nj \
	      --pdf=${test_ivec_dir}/gmm_pdf.mat \
	      --dim=$lda_dim --total-covariance-factor=$covar_factor \
    	"ark:ivector-normalize-length scp:${plda_ivec_dir}/ivector.scp ark:- |	transform-vec  ${plda_ivec_dir}/transform_lda.mat ark:- ark:- | ivector-normalize-length ark:-  ark:- |" \
			ark:${plda_data_dir}/utt2spk \
			ark:${plda_ivec_dir}/transform_lda_lc.ark \
			ark,t:${plda_ivec_dir}/spk_mean_lda.ark || exit 1;
	
	# it contain the total mean at the last row
	run.pl ${plda_ivec_dir}/log/spk_mean_ln_lda.log \
	...
	$train_cmd $plda_ivec_dir/log/lda_lclda_plda3.log \
	    ivector-compute-plda-set --nj=$nj \
	      ...
	      ark:${plda_ivec_dir}/transform_lda_lc.ark  \
		"ark:ivector-normalize-length scp:${plda_ivec_dir}/ivector.scp ark:- |	transform-vec  ${plda_ivec_dir}/transform_lda.mat ark:- ark:- | ivector-normalize-length ark:-  ark:- |" \
	      ark:$plda_ivec_dir/lda_lclda_plda_set.ark || exit 1;
...
$train_cmd $scores_dir/log/eval_scoring.log \
	ivector-lclda-pldaset-scoring --normalize-length=true \
	  ...
	  --transform=ark:${plda_ivec_dir}/transform_lda_lc.ark \
	  --spk-mean=ark:${plda_ivec_dir}/spk_mean_ln_lda.ark \
	  ark:"$plda_ivec_dir/lda_lclda_plda_set.ark" \
	  ark:"ivector-mean ark:$enroll_data_dir/spk2utt scp:$enroll_ivec_dir/ivector.scp ark:- | ivector-subtract-global-mean --subtract-mean=false $plda_ivec_dir/mean.vec ark:- ark:- | transform-vec ${plda_ivec_dir}/transform_lda.mat ark:- ark:- |" \
	  ark:"ivector-subtract-global-mean --subtract-mean=false $plda_ivec_dir/mean.vec scp:$test_ivec_dir/ivector.scp ark:- | transform-vec  ${plda_ivec_dir}/transform_lda.mat ark:- ark:- |" \
	...
	# --simple-length-normalization=$simple_length_norm \
	# --num-utts=ark:$enroll_ivec_dir/num_utts.ark \
	# --transform=ark:${plda_ivec_dir}/transform_${lda_dim}.ark \
