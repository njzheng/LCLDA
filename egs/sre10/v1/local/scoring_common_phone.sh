...
# data/sre10_train 
# data/sre10_test \
# exp/subivectors_sre_dnn80/phone_vector
# exp/subivectors_sre10_train_dnn80/phone_vector
# exp/subivectors_sre10_test_dnn80/phone_vector
row_indx=0
...
if [ $# != 6 ]; then
  echo "Usage: $0 --row-indx 0 <plda-data-dir> <enroll-data-dir> <test-data-dir> <plda-pvec-dir> <enroll-pvec-dir> <test-pvec-dir>"
fi
# delet '/' in the path
plda_data_dir=${1%/}
enroll_data_dir=${2%/}
...
phone_vectors_file=phone_vectors.$row_indx.scp
spk_phone_vectors_file=spk_phone_vectors.$row_indx.scp
for j in ${enroll_ivec_dir} ${test_ivec_dir} ${plda_ivec_dir}; do
  if [ ! -f ${j}/${phone_vectors_file} ]; then
    echo "No file in ${j}/${phone_vectors_file}" && exit 1;
  fi

  if [ ! -f ${j}/${spk_phone_vectors_file} ]; then
    echo "No file in ${j}/${spk_phone_vectors_file}" && exit 1;
  fi
done
...
utils/filter_scp.pl ${enroll_data_dir}_male/utt2spk \
  ${enroll_ivec_dir}/${phone_vectors_file} > ${enroll_ivec_dir}_male/${phone_vectors_file}
utils/filter_scp.pl ${test_data_dir}_male/utt2spk \
  ${test_ivec_dir}/${phone_vectors_file} > ${test_ivec_dir}_male/${phone_vectors_file}
utils/filter_scp.pl ${enroll_data_dir}_female/utt2spk \
  ${enroll_ivec_dir}/${phone_vectors_file} > ${enroll_ivec_dir}_female/${phone_vectors_file}
utils/filter_scp.pl ${test_data_dir}_female/utt2spk \
  ${test_ivec_dir}/${phone_vectors_file} > ${test_ivec_dir}_female/${phone_vectors_file}
utils/filter_scp.pl ${plda_data_dir}_female/utt2spk \
  ${plda_ivec_dir}/${phone_vectors_file} > ${plda_ivec_dir}_female/${phone_vectors_file}
utils/filter_scp.pl ${plda_data_dir}_male/utt2spk \
  ${plda_ivec_dir}/${phone_vectors_file} > ${plda_ivec_dir}_male/${phone_vectors_file}

utils/filter_scp.pl ${enroll_data_dir}_male/spk2utt \
  ${enroll_ivec_dir}/${spk_phone_vectors_file} > ${enroll_ivec_dir}_male/${spk_phone_vectors_file}
utils/filter_scp.pl ${enroll_data_dir}_female/spk2utt \
  ${enroll_ivec_dir}/${spk_phone_vectors_file} > ${enroll_ivec_dir}_female/${spk_phone_vectors_file}
utils/filter_scp.pl ${enroll_data_dir}_male/spk2utt \
...
utils/filter_scp.pl ${plda_data_dir}_male/spk2utt \
  ${plda_ivec_dir}/${spk_phone_vectors_file} > ${plda_ivec_dir}_male/${spk_phone_vectors_file}
utils/filter_scp.pl ${plda_data_dir}_female/spk2utt \
  ${plda_ivec_dir}/${spk_phone_vectors_file} > ${plda_ivec_dir}_female/${spk_phone_vectors_file}
utils/filter_scp.pl ${plda_data_dir}_male/spk2utt \
...
run.pl ${plda_ivec_dir}/log/compute_mean.${row_indx}.log \
  ivector-normalize-length scp:${plda_ivec_dir}/${phone_vectors_file}  \
  ark:- \| ivector-mean ark:- ${plda_ivec_dir}/mean.${row_indx}.vec || exit 1;
run.pl ${plda_ivec_dir}_male/log/compute_mean.${row_indx}.log \
  ivector-normalize-length scp:${plda_ivec_dir}_male/${phone_vectors_file}  \
  ark:- \| ivector-mean ark:- ${plda_ivec_dir}_male/mean.${row_indx}.vec || exit 1;
run.pl ${plda_ivec_dir}_female/log/compute_mean.${row_indx}.log \
  ivector-normalize-length scp:${plda_ivec_dir}_female/${phone_vectors_file}  \
  ark:- \| ivector-mean ark:- ${plda_ivec_dir}_female/mean.${row_indx}.vec || exit 1;
