// subivector/subivector-extractor.cc
...
int32 SubIvectorExtractor::FeatDim() const {
  KALDI_ASSERT(!M_.empty());
  return M_[0].NumRows();
}

int32 SubIvectorExtractor::IvectorDim() const {
  if (M_.empty()) { return 0.0; }
  else { return M_[0].NumCols(); }
}

int32 SubIvectorExtractor::NumGauss() const {
  return static_cast<int32>(M_.size());
}

int32 SubIvectorExtractor::PhoneDim() const {
  return static_cast<int32>(prior_offset_vec_.Dim());
}
...
// otherwise occur.
// static
void SubIvectorExtractor::InvertWithFlooring(const SpMatrix<double> &inverse_var,
                                          SpMatrix<double> *var) {
...
void SubIvectorExtractor::GetIvectorDistribution(
...
// newly added
void SubIvectorExtractor::GetSubIvectorDistribution(
    const SubIvectorExtractorUtteranceStats &utt_stats,
    ...
    const int32 phone_num_) const 
{
  if (!IvectorDependentWeights()) {
    ...
    for(int32 i=0; i< phone_num_; i++){
      // for each subivector we can get a linear and quadratic
      Vector<double> linear(IvectorDim());
      SpMatrix<double> quadratic(IvectorDim());
      
      // find the matched indeces of the phone i in triphone list
      std::vector<int32> triphone_index;
      for(int32 indx=0; indx < ph_tri_list_.size(); indx++)
      {
        if(ph_tri_list_[indx]==i)
          triphone_index.push_back(indx);
      }
      KALDI_ASSERT(static_cast<int32>(triphone_index.size()) > 0);

      // GetIvectorDistMean(utt_stats, &linear, &quadratic);
      // GetIvectorDistPrior(utt_stats, &linear, &quadratic);
      GetSubIvectorDistMean(utt_stats, &linear, &quadratic, triphone_index);
      GetSubIvectorDistPrior(utt_stats, &linear, &quadratic, i);   


      // a reference
      SubVector<double> mean(mean_mat, i); // get i-th row of mean_mat.

      // whether vmat is empty
      if (var_vmat.size() != 0) {
        var_vmat[i].CopyFromSp(quadratic);
        var_vmat[i].Invert(); // now it's a variance.
        // mean of distribution = quadratic^{-1} * linear...
        mean.AddSpVec(1.0, var_vmat[i], linear, 0.0);
      } else {
        quadratic.Invert();
        mean.AddSpVec(1.0, quadratic, linear, 0.0);
      }
  ...
  {
    KALDI_ERR << "Write failure in IvectorDependentWeights";
    for(int32 i=0; i< phone_num_; i++)
    {
      Vector<double> linear(IvectorDim());
      SpMatrix<double> quadratic(IvectorDim());
      ...
      // build a reference of var_vmat[i]
      if (var_vmat.size() != 0)
        SpMatrix<double> var = var_vmat[i];
      // maybe a pointer, i am not sure
      SubVector<double> mean(mean_mat, i); // get i-th row of mean_mat.
      ...
      for (int32 iter = 0; iter < num_iters; iter++) {
        if (GetVerboseLevel() >= 3) {
          // I am not very understanding here!
          KALDI_VLOG(3) << "Auxf on iter " << iter << " is "
                        << GetSubAuxf(utt_stats, cur_mean, triphone_index, &quadratic_inv);
          int32 show_dim = 5;
          if (show_dim > cur_mean.Dim()) show_dim = cur_mean.Dim();
          KALDI_VLOG(3) << "Current distribution mean is "
                        << cur_mean.Range(0, show_dim) << "... "
        ...
        // modified
        GetSubIvectorDistWeight(utt_stats, cur_mean,
                             &this_linear, &this_quadratic,triphone_index);
        ...
        mean_diff.AddVec(-1.0, cur_mean);
        double change = mean_diff.Norm(2.0);
        KALDI_VLOG(2) << "On iter " << iter << ", SubiVector changed by " << change;
        if (change < change_threshold)
          break;
      }
      ...
      mean.CopyFromVec(cur_mean);
      if (var_vmat.size() != 0)
        var_vmat[i].CopyFromSp(quadratic_inv);
    }
  }
}
...
SubIvectorExtractor::SubIvectorExtractor(
  ...
  // newly added
  prior_offset_vec_.Resize(phone_num_);
  prior_offset_vec_.SetZero();
  prior_offset_vec_.Add(100.0);

  M_.resize(num_gauss);
  for (int32 i = 0; i < num_gauss; i++) {
    M_[i].Resize(feature_dim, opts.ivector_dim);
    M_[i].SetRandn();
    // copy gmm_means.Row(i) into the 0-th column of M_[i]
    // that is M_[i] * ivector = M_[i](:,1:end-1)*ivector(1:end-1)
    // + M_[i](:,0)*ivector(0) --> the speaker adpated mean of feats
...
///newly added , but the parameter in ivector is protected
      ...
      int32 phone_num_) 
{
  ...
  // copy the parameters from totextractor into subextractor
  int32 feature_dim = totextractor.FeatDim();
  int32 ivector_dim = totextractor.IvectorDim();
  int32 num_gauss = totextractor.NumGauss();
  KALDI_ASSERT(opts.ivector_dim == ivector_dim);
  KALDI_ASSERT(num_gauss > 0);
  KALDI_ASSERT(feature_dim > 0);

  Sigma_inv_.resize(num_gauss);
  for (int32 i = 0; i < num_gauss; i++) {
    // const SpMatrix<BaseFloat> &inv_var = fgmm.inv_covars()[i];
    // int32 numrows = feature_dim; //totextractor.Sigma_inv_[0].NumRows();
    // Sigma_inv_[i].Resize(numrows);
  ...
  prior_offset_vec_.Resize(phone_num_);
  prior_offset_vec_.SetZero();
  prior_offset_vec_.Add(prior_offset_);



  M_.resize(num_gauss);
  for (int32 i = 0; i < num_gauss; i++) {
    // copy from totextractor
    // M_[i].Resize(feature_dim, opts.ivector_dim);
...
class SubIvectorExtractorComputeDerivedVarsClass {
 public:
  SubIvectorExtractorComputeDerivedVarsClass(SubIvectorExtractor *extractor,
                                          int32 i):
      extractor_(extractor), i_(i) { }
  void operator () () { extractor_->ComputeDerivedVars(i_); }
 private:
  SubIvectorExtractor *extractor_;
  int32 i_;
};

void SubIvectorExtractor::ComputeDerivedVars() {
  KALDI_LOG << "Computing derived variables for sub iVector extractor";
  ...
  // could because some tasks finish before others.
  {
    ...
    TaskSequencer<SubIvectorExtractorComputeDerivedVarsClass> sequencer(
        sequencer_opts);
    for (int32 i = 0; i < NumGauss(); i++)
      sequencer.Run(new SubIvectorExtractorComputeDerivedVarsClass(this, i));
  }
  KALDI_LOG << "Done.";
}


void SubIvectorExtractor::ComputeDerivedVars(int32 i) {
...
void SubIvectorExtractor::GetIvectorDistWeight(
    const SubIvectorExtractorUtteranceStats &utt_stats,
...
void SubIvectorExtractor::GetSubIvectorDistWeight(
    const SubIvectorExtractorUtteranceStats &utt_stats,
    const VectorBase<double> &mean,
    VectorBase<double> *linear,
  ...
  // not sure here?
  double gamma = 0;
  for (int32 iter = 0; iter < static_cast<int32>(triphone_index.size()); iter++) {
    int32 i = triphone_index[iter];
    gamma += utt_stats.gamma_(i);
  }
  for (int32 iter = 0; iter < static_cast<int32>(triphone_index.size()); iter++) {
    int32 i = triphone_index[iter];
    double gamma_i = utt_stats.gamma_(i);
...
void SubIvectorExtractor::GetIvectorDistMean(
    const SubIvectorExtractorUtteranceStats &utt_stats,
    VectorBase<double> *linear,
    SpMatrix<double> *quadratic) const {
  int32 I = NumGauss();
...
void SubIvectorExtractor::GetSubIvectorDistMean(
    const SubIvectorExtractorUtteranceStats &utt_stats,
    VectorBase<double> *linear,
    SpMatrix<double> *quadratic,
    const std::vector<int32> &triphone_index) const 
{
  ...
  // q_vec = vectorize(quadratic): == U^T *gamma
  SubVector<double> q_vec(quadratic->Data(), IvectorDim()*(IvectorDim()+1)/2);


  // for (int32 i = 0; i < I; i++) {
  for (int32 indx = 0;indx < static_cast<int32>(triphone_index.size()); indx++)
  {
    int32 i = triphone_index[indx];
    ...
    // select the i-th rows of U_ to time with gamma(i)
    // q_vec.AddMatVec(1.0, U_, kTrans, utt_stats.gamma_, 1.0);
...
void SubIvectorExtractor::GetIvectorDistPrior(
    const SubIvectorExtractorUtteranceStats &utt_stats,
    VectorBase<double> *linear,
    SpMatrix<double> *quadratic) const {
...
void SubIvectorExtractor::GetSubIvectorDistPrior(
    const SubIvectorExtractorUtteranceStats &utt_stats,
    VectorBase<double> *linear,
    SpMatrix<double> *quadratic,
    int32 phone_indx) const {
  // the zero'th dimension has an offset mean.
  KALDI_ASSERT(phone_indx < prior_offset_vec_.Dim());
  (*linear)(0) += prior_offset_vec_(phone_indx);
  /// The inverse-variance for the prior is the unit matrix.
  quadratic->AddToDiag(1.0);
}

double SubIvectorExtractor::GetAcousticAuxfWeight(
    const SubIvectorExtractorUtteranceStats &utt_stats,
    const VectorBase<double> &mean,
...
double SubIvectorExtractor::GetAuxf(const SubIvectorExtractorUtteranceStats &utt_stats,
                                 const VectorBase<double> &mean,
                                 const SpMatrix<double> *var) const {
...
// newly added
double SubIvectorExtractor::GetSubAuxf(const SubIvectorExtractorUtteranceStats &utt_stats,
                                      const VectorBase<double> &mean,
                                      const std::vector<int32> &triphone_index,
                                      const SpMatrix<double> *var) const 
{

  double acoustic_auxf = GetAcousticSubAuxf(utt_stats, mean, triphone_index, var);
  double prior_auxf = GetPriorAuxf(mean, var);
  double num_frames = utt_stats.gamma_.Sum();
  KALDI_VLOG(3) << "Acoustic auxf is " << (acoustic_auxf/num_frames) << "/frame over "
                << num_frames << " frames, prior auxf is " << prior_auxf
                << " = " << (prior_auxf/num_frames) << " per frame.";
  return acoustic_auxf + prior_auxf;
...
/*
  ...
  more natural.
*/
double SubIvectorExtractor::GetPriorAuxf(
    const VectorBase<double> &mean,
    const SpMatrix<double> *var) const {
...
*/
double SubIvectorExtractor::GetAcousticAuxf(
    const SubIvectorExtractorUtteranceStats &utt_stats,
    const VectorBase<double> &mean,
    const SpMatrix<double> *var) const {
...
double SubIvectorExtractor::GetAcousticSubAuxf(
    const SubIvectorExtractorUtteranceStats &utt_stats,
    const VectorBase<double> &mean,
    const std::vector<int32> &triphone_index,
    const SpMatrix<double> *var) const{
...
/* This is the part of the auxf that involves the data
   ...
   above accordingly.
*/
double SubIvectorExtractor::GetAcousticAuxfMean(
    const SubIvectorExtractorUtteranceStats &utt_stats,
    const VectorBase<double> &mean,
    const SpMatrix<double> *var) const {
  double K = 0.0;
...
double SubIvectorExtractor::GetAcousticAuxfGconst(
...
double SubIvectorExtractor::GetAcousticAuxfVariance(
    const SubIvectorExtractorUtteranceStats &utt_stats) const {
...
void SubIvectorExtractor::TransformIvectors(const MatrixBase<double> &T,
...
void SubIvectorExtractor::TransformSubIvectors(
  const std::vector<Matrix<double> > &T_vec,
  const Vector<double> &offset_vec,
  const std::vector<int32> &ph_tri_list_){


  for(int32 i=0; i< T_vec.size(); i++){
    Matrix<double> Tinv(T_vec[i]);
    Tinv.Invert();
    // w <-- w Tinv.  (construct temporary copy with Matrix<double>(w))
    if (IvectorDependentWeights())
      KALDI_ERR << "Write failure in IvectorDependentWeights";
    ...
    for(int32 indx=0; indx < ph_tri_list_.size(); indx++)
    {
      // next: M_i <-- M_i Tinv.  (construct temporary copy with Matrix<double>(M_[i]))
      if(ph_tri_list_[indx]==i){
        M_[indx].AddMatMat(1.0, Matrix<double>(M_[indx]), kNoTrans, Tinv, kNoTrans, 0.0);
      }
    }

  }

  // note sure that offset_vec can be cout 
  KALDI_LOG << "Setting SubiVector prior offset vec to " << offset_vec;
  prior_offset_vec_ = offset_vec;
...
void SubIvectorExtractor::Write(std::ostream &os, bool binary) const {
  WriteToken(os, binary, "<SubIvectorExtractor>");
  ...
  WriteToken(os, binary, "<SubIvectorOffset_vec>");
  prior_offset_vec_.Write(os, binary);
  WriteToken(os, binary, "</SubIvectorExtractor>");

}

void SubIvectorExtractor::Write_M(std::ostream &os, bool binary) const {
  // WriteToken(os, binary, "<SubIvectorExtractor>");
  // WriteToken(os, binary, "<M>");
  int32 size = M_.size();
  // WriteBasicType(os, binary, size);
...
void SubIvectorExtractor::Read(std::istream &is, bool binary) {
  ExpectToken(is, binary, "<SubIvectorExtractor>");
  ...
  ExpectToken(is, binary, "<SubIvectorOffset_vec>");
  prior_offset_vec_.Read(is, binary);
  ExpectToken(is, binary, "</SubIvectorExtractor>");
  ComputeDerivedVars();
}

void SubIvectorExtractor::ReadfromIvectorExtractor(
    std::istream &is, bool binary,int32 phone_num_) {
...
void SubIvectorExtractorUtteranceStats::AccStats(
...
void SubIvectorExtractorUtteranceStats::Get_phone_gamma(Vector<double> &phone_gamma_, const std::vector<int32> &ph_tri_list_)
{
  for  (int32 i = 0; i < gamma_.Dim(); i++) {
    // get the phone index for subivec_mean_mat
    phone_gamma_(ph_tri_list_[i]) += gamma_(i);
  }
...
void SubIvectorExtractorUtteranceStats::Scale(double scale) {
  gamma_.Scale(scale);
  X_.Scale(scale);
...
SubIvectorExtractorStats::SubIvectorExtractorStats(
    const SubIvectorExtractor &extractor,
  ...
  // store the vector list in the extractor
  KALDI_ASSERT(I == static_cast<int32>(phone_triphone_list.size())  && "Dimension mismatch in phone_triphone_list and extractor");

  ph_tri_list_= phone_triphone_list;
  // the number of the monophones
  // std::max_element return a iterator
  this->phone_num_ = *(std::max_element(ph_tri_list_.begin(), ph_tri_list_.end())) + 1;

  // newly added
  subivector_mat_sum_.Resize(phone_num_,S);
  subivector_vmat_scatter_.resize(phone_num_);
  for (int32 i = 0; i < phone_num_; i++)
    subivector_vmat_scatter_[i].Resize(S);

}


void SubIvectorExtractorStats::CommitStatsForM(
    const SubIvectorExtractor &extractor,
    const SubIvectorExtractorUtteranceStats &utt_stats,
...
// newly added
void SubIvectorExtractorStats::CommitSubStatsForM(const SubIvectorExtractor &extractor,
                       const SubIvectorExtractorUtteranceStats &utt_stats,
                       ...
                       const std::vector< SpMatrix<double> > &subivec_var_vmat)
{
  ...
  for  (int32 i = 0; i < extractor.NumGauss(); i++) {
    ...
    Vector<double> subivec_mean(subivec_mean_mat.Row(phone_indx));
    Y_[i].AddVecVec(1.0, utt_stats.X_.Row(i),subivec_mean);
  }
  gamma_Y_lock_.unlock();

  // need specify subivec_var
  // SpMatrix<double> ivec_scatter(ivec_var);
  // ivec_scatter.AddVec2(1.0, ivec_mean);
  ...
  // }
  // R_gamma_cache_.Row(R_num_cached_).CopyFromVec(utt_stats.gamma_);
  // int32 ivector_dim = ivec_mean.Dim(); //S
  // SubVector<double> ivec_scatter_vec(ivec_scatter.Data(),
  //                                    ivector_dim * (ivector_dim + 1) / 2);
  // R_ivec_scatter_cache_.Row(R_num_cached_).CopyFromVec(ivec_scatter_vec);
  // R_num_cached_++;
  ...
  int32 subivector_dim = subivec_mean_mat.NumCols() ; //S
  // // used for store subivec_scatter_vec for each tirphone index
  // Matrix<double> R_ivec_scatter_mat(extractor.NumGauss(),
  //                                   subivector_dim * (subivector_dim + 1) / 2);
  ...
  for (int32 i = 0; i < extractor.NumGauss(); i++) {
    ...
    SpMatrix<double> subivec_scatter(subivec_var_vmat[phone_indx]);
    Vector<double> subivec_mean(subivec_mean_mat.Row(phone_indx));


    subivec_scatter.AddVec2(1.0, subivec_mean);
    // vectorize
    SubVector<double> subivec_scatter_vec(subivec_scatter.Data(),
                               subivector_dim * (subivector_dim + 1) / 2);
    
    // make a reference of R_
...
void SubIvectorExtractorStats::FlushCache() {
  // no use here
  R_cache_lock_.lock();
  if (R_num_cached_ > 0) {
    ...
    Matrix<double> R_gamma_cache( //Return a sub-part of matrix.
...
void SubIvectorExtractorStats::CommitStatsForSigma(
    const SubIvectorExtractor &extractor,
    const SubIvectorExtractorUtteranceStats &utt_stats) {
...
void SubIvectorExtractorStats::CommitStatsForWPoint(
    const SubIvectorExtractor &extractor,
    const SubIvectorExtractorUtteranceStats &utt_stats,
...
void SubIvectorExtractorStats::CommitStatsForW(
    const SubIvectorExtractor &extractor,
    const SubIvectorExtractorUtteranceStats &utt_stats,
    const VectorBase<double> &ivec_mean,
    const SpMatrix<double> &ivec_var) {
...
void SubIvectorExtractorStats::CommitStatsForPrior(
    const VectorBase<double> &ivec_mean,
    const SpMatrix<double> &ivec_var) {
  SpMatrix<double> ivec_scatter(ivec_var);
  ivec_scatter.AddVec2(1.0, ivec_mean);
...
// newly added
void SubIvectorExtractorStats::CommitSubStatsForPrior(
  const Matrix<double> &subivec_mean_mat,
  const std::vector< SpMatrix<double> > &subivec_var_vmat){
  
  // update the sum of sub ivector mean and variance
  prior_stats_lock_.lock();

  subivector_mat_sum_.AddMat(1.0, subivec_mean_mat);

  for(int32 i=0; i < phone_num_; i++){
    SpMatrix<double> subivec_scatter(subivec_var_vmat[i]);
    subivec_scatter.AddVec2(1.0, subivec_mean_mat.Row(i));
    
    // // build a reference of subivector_mat_sum_ Row(i)
    // SubVector<double> subivector_sum_(subivector_mat_sum_,i);
    // subivector_sum_.AddVec(1.0, subivec_mean_mat.Row(i)); 
    // // not sure if need to convert into Vector<double>(subivec_mean_mat.Row(i))
    
    subivector_vmat_scatter_[i].AddSp(1.0, subivec_scatter);
  }
...
void SubIvectorExtractorStats::CommitStatsForUtterance(
    const SubIvectorExtractor &extractor,
    const SubIvectorExtractorUtteranceStats &utt_stats) {
  ...
  // compute the L^-1 and ivector mean square in each utterance, 
  // and then sequencely sum them up
...
// modified from CommitStatsForUtterance, comput the set of subivectors
void SubIvectorExtractorStats::CommitSubStatsForUtterance(
    const SubIvectorExtractor &extractor,
    const SubIvectorExtractorUtteranceStats &utt_stats) {

  int32 ivector_dim = extractor.IvectorDim();
  // Vector<double> ivec_mean(ivector_dim);
  // each row subvector is a subivector
  Matrix<double> subivec_mean_mat(phone_num_,ivector_dim);

  // SpMatrix<double> ivec_var(ivector_dim);
  std::vector< SpMatrix<double> > subivec_var_vmat;
  subivec_var_vmat.resize(phone_num_);
  for (int32 i = 0; i < phone_num_; i++)
    subivec_var_vmat[i].Resize(ivector_dim); 

  // extractor.GetIvectorDistribution(utt_stats,&ivec_mean,&ivec_var);
  extractor.GetSubIvectorDistribution(utt_stats, subivec_mean_mat, subivec_var_vmat,
                                      ph_tri_list_, phone_num_);

  // leave to complete
  if (false){//config_.compute_auxf){
    // cal tot_auxf for each phone
    for(int32 i=0; i< phone_num_; i++)
    {
      // build a copy of var_vmat[i]
      SpMatrix<double> ivec_var = subivec_var_vmat[i];
      // maybe a pointer, i am not sure
      SubVector<double> ivec_mean(subivec_mean_mat, i); // get i-th row of mean_mat.
      ...
      // tot_auxf_ += extractor.GetAuxf(utt_stats, ivec_mean, &ivec_var);
      tot_auxf_ += extractor.GetSubAuxf(utt_stats, ivec_mean, triphone_index,&ivec_var);
    }
  ...
  // and then sequencely sum them up
  // CommitStatsForM(extractor, utt_stats, ivec_mean, ivec_var);
  CommitSubStatsForM(extractor, utt_stats, subivec_mean_mat, subivec_var_vmat);
  
  if (extractor.IvectorDependentWeights())
    KALDI_ERR << "Write failure in IvectorDependentWeights";// not update weight
    //CommitStatsForW(extractor, utt_stats, ivec_mean, ivec_var);

  // CommitStatsForPrior(ivec_mean, ivec_var);
  CommitSubStatsForPrior(subivec_mean_mat, subivec_var_vmat);

  if (!S_.empty())// remain unchanged here
    CommitStatsForSigma(extractor, utt_stats);
}



void SubIvectorExtractorStats::CheckDims(const SubIvectorExtractor &extractor) const {
  int32 S = extractor.IvectorDim(), D = extractor.FeatDim(),
      I = extractor.NumGauss();
  ...
  // newly added
  KALDI_ASSERT(subivector_mat_sum_.NumCols() == S);
  KALDI_ASSERT(subivector_vmat_scatter_[0].NumRows() == S);
  
}

void SubIvectorExtractorStats::AccStatsForUtterance(
    const SubIvectorExtractor &extractor,
    const MatrixBase<BaseFloat> &feats,
    const Posterior &post) {
  typedef std::vector<std::pair<int32, BaseFloat> > VecType;
  ...
  SubIvectorExtractorUtteranceStats utt_stats(num_gauss, feat_dim,
                                           update_variance);

  utt_stats.AccStats(feats, post);

  // CommitStatsForUtterance(extractor, utt_stats);
  // add some parameters in SubIvectorExtractor
  CommitSubStatsForUtterance(extractor, utt_stats);

}

double SubIvectorExtractorStats::AccStatsForUtterance(
    const SubIvectorExtractor &extractor,
    const MatrixBase<BaseFloat> &feats,
    const FullGmm &fgmm) {
  int32 num_frames = feats.NumRows();
...
void SubIvectorExtractorStats::Add(const SubIvectorExtractorStats &other) {
  ...
  //newly added
  subivector_mat_sum_.AddMat(weight, other.subivector_mat_sum_);
  for (size_t i = 0; i < subivector_vmat_scatter_.size(); i++)
    subivector_vmat_scatter_[i].AddSp(weight, other.subivector_vmat_scatter_[i]);
}


void SubIvectorExtractorStats::Write(std::ostream &os, bool binary) {
  FlushCache(); // for R stats.
  ((const SubIvectorExtractorStats&)(*this)).Write(os, binary); // call const version.
}


void SubIvectorExtractorStats::Write(std::ostream &os, bool binary) const {
  ...
  //newly added  
  WriteToken(os, binary, "<SubIvectorMatSum>");
  subivector_mat_sum_.Write(os, binary);
  WriteToken(os, binary, "<SubIvectorVmatScatter>");
  size = subivector_vmat_scatter_.size(); // for vmat size read
  WriteBasicType(os, binary, size);
  for (int32 i = 0; i < size; i++)
    subivector_vmat_scatter_[i].Write(os, binary);

  WriteToken(os, binary, "<PhoneDim>");
  WriteBasicType(os, binary, phone_num_);

  WriteToken(os, binary, "<PhTriList>");
  Vector<float> ph_tri_list_f;
  ph_tri_list_f.Resize(ph_tri_list_.size()); 
  // convert std::vector<int32> into Vector<float> 
  for (int32 i=0; i<ph_tri_list_.size();i++){
    ph_tri_list_f(i) = static_cast<float>(ph_tri_list_[i]);
  }
  ph_tri_list_f.Write(os, binary);
...
void SubIvectorExtractorStats::Read(std::istream &is, bool binary, bool add) {
  ...
  //newly added  
  ExpectToken(is, binary, "<SubIvectorMatSum>");
  subivector_mat_sum_.Read(is, binary, add);
  ExpectToken(is, binary, "<SubIvectorVmatScatter>");
  ReadBasicType(is, binary, &size);
  subivector_vmat_scatter_.resize(size);
  for (int32 i = 0; i < size; i++)
    subivector_vmat_scatter_[i].Read(is, binary, add);


  ExpectToken(is, binary, "<PhoneDim>");
  ReadBasicType(is, binary, &phone_num_);

  ExpectToken(is, binary, "<PhTriList>");
  Vector<float> ph_tri_list_f;
  ph_tri_list_f.Read(is, binary, false); // not add
  ph_tri_list_.resize(ph_tri_list_f.Dim());
  // convert Vector<float> into std::vector<int32>
  for (int32 i=0; i<ph_tri_list_f.Dim();i++){
    ph_tri_list_[i] = static_cast<int32>(ph_tri_list_f(i));
  }
...
double SubIvectorExtractorStats::Update(
    const SubIvectorExtractorEstimationOptions &opts,
  ...
  double ans = 0.0;
  // update M (subtotal varible space)
  ans += UpdateProjections(opts, extractor); 

  // do not udpate weights
  if (extractor->IvectorDependentWeights())
    KALDI_ERR << "Write failure in IvectorDependentWeights";
...
void SubIvectorExtractorStats::IvectorVarianceDiagnostic(
  const SubIvectorExtractor &extractor) {
...
double SubIvectorExtractorStats::UpdateProjection(
    const SubIvectorExtractorEstimationOptions &opts,
    int32 i,
    SubIvectorExtractor *extractor) const {
  ...
  SubVector<double> R_vec(R_, i); // i'th row of R; vectorized form of SpMatrix. LU
...
void SubIvectorExtractorStats::GetOrthogonalIvectorTransform(
...
class SubIvectorExtractorUpdateProjectionClass {
 public:
  SubIvectorExtractorUpdateProjectionClass(const SubIvectorExtractorStats &stats,
                        const SubIvectorExtractorEstimationOptions &opts,
                        int32 i,
                        SubIvectorExtractor *extractor,
                        double *tot_impr):
  ...
  ~SubIvectorExtractorUpdateProjectionClass() { *tot_impr_ += impr_; }
 private:
  const SubIvectorExtractorStats &stats_;
  const SubIvectorExtractorEstimationOptions &opts_;
  int32 i_;
  SubIvectorExtractor *extractor_;
  double *tot_impr_;
  double impr_;
...
double SubIvectorExtractorStats::UpdateProjections(
    const SubIvectorExtractorEstimationOptions &opts,
    SubIvectorExtractor *extractor) const {
  ...
  double tot_impr = 0.0;
  {
    TaskSequencerConfig sequencer_opts;
    sequencer_opts.num_threads = g_num_threads;
    TaskSequencer<SubIvectorExtractorUpdateProjectionClass> sequencer(
        sequencer_opts);
    for (int32 i = 0; i < I; i++)
      sequencer.Run(new SubIvectorExtractorUpdateProjectionClass(
...
double SubIvectorExtractorStats::UpdateVariances(
    const SubIvectorExtractorEstimationOptions &opts,
    SubIvectorExtractor *extractor) const {
  int32 num_gauss = extractor->NumGauss(),
...
double SubIvectorExtractorStats::UpdateWeight(
    const SubIvectorExtractorEstimationOptions &opts,
    int32 i,
    SubIvectorExtractor *extractor) const {
...
class SubIvectorExtractorUpdateWeightClass {
  // no use here
 public:
  SubIvectorExtractorUpdateWeightClass(const SubIvectorExtractorStats &stats,
                                    const SubIvectorExtractorEstimationOptions &opts,
                                    int32 i,
                                    SubIvectorExtractor *extractor,
                                    double *tot_impr):
  ...
  ~SubIvectorExtractorUpdateWeightClass() { *tot_impr_ += impr_; }
 private:
  const SubIvectorExtractorStats &stats_;
  const SubIvectorExtractorEstimationOptions &opts_;
  int32 i_;
...
double SubIvectorExtractorStats::UpdateWeights(
    const SubIvectorExtractorEstimationOptions &opts,
    SubIvectorExtractor *extractor) const {
  ...
  double tot_impr = 0.0;
  {
    TaskSequencerConfig sequencer_opts;
    sequencer_opts.num_threads = g_num_threads;
    TaskSequencer<SubIvectorExtractorUpdateWeightClass> sequencer(
        sequencer_opts);
    for (int32 i = 0; i < I; i++)
      sequencer.Run(new SubIvectorExtractorUpdateWeightClass(
          *this, opts, i, extractor, &tot_impr));
  }
...
double SubIvectorExtractorStats::PriorDiagnostics(double old_prior_offset) const {
...
double SubIvectorExtractorStats::UpdatePrior(
    const SubIvectorExtractorEstimationOptions &opts,
    SubIvectorExtractor *extractor) const {

  KALDI_ASSERT(num_ivectors_ > 0.0);
  KALDI_LOG << "num_ivectors_ is  " << num_ivectors_;
  // Vector<double> sum(ivector_sum_);
  // sum.Scale(1.0 / num_ivectors_);
  // SpMatrix<double> covar(ivector_scatter_);
  // covar.Scale(1.0 / num_ivectors_);
  ...
  if (static_cast<int32>(subivector_vmat_scatter_.size())!=phone_num_){
    KALDI_LOG<< " the number of phone and vmat_scatter unmatched"
              <<static_cast<int32>(subivector_vmat_scatter_.size()) <<"; "<<phone_num_;
    KALDI_ASSERT(static_cast<int32>(subivector_vmat_scatter_.size())== phone_num_);
  }

  int32 ivector_dim = extractor->IvectorDim();

  // used to store V
  std::vector< Matrix<double> > V_vec;
  V_vec.resize(phone_num_);
  for (int32 i = 0; i < phone_num_; i++)
      V_vec[i].Resize(ivector_dim, ivector_dim);

  // used to store sum_vproj(0)
  Vector<double> sum_vproj_vec(phone_num_);

  for(int32 i=0; i< phone_num_; i++)
  {

    Vector<double> sum(subivector_mat_sum_.Row(i));
    sum.Scale(1.0 / num_ivectors_);
    SpMatrix<double> covar(subivector_vmat_scatter_[i]);
    covar.Scale(1.0 / (num_ivectors_-1.0) );
    covar.AddVec2(-1.0, sum); // Get the centered covariance.
    ...
    V_vec[i].CopyFromMat(V, kNoTrans);
    sum_vproj_vec(i)=sum_vproj(0);
  }

// ignore here
  double ans = 0.0; //PriorDiagnostics(extractor->prior_offset_);
...
SubIvectorExtractorStats::SubIvectorExtractorStats (
    const SubIvectorExtractorStats &other):
    ph_tri_list_(other.ph_tri_list_), phone_num_(other.phone_num_),
    ...
    subivector_mat_sum_(other.subivector_mat_sum_), 
